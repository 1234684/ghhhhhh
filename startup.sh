...
/usr/bin/xray -config /root/config.json & /root/cloudflared tunnel --no-autoupdate run --token $TUNNEL_TOKEN & nginx -c /root/nginx.conf -g 'daemon off;'
else
/usr/bin/xray -config /root/config.json & nginx -c /root/nginx.conf -g 'daemon off;'
fi
